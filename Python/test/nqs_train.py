...
try:
    ...
    from QES.general_python.common.flog import Logger, log_timing_summary, print_arguments
...
def single_step_train(i: int, lr: float, reset=False):
    ...
    return (step_info.mean_energy, step_info.std_energy), (
        time_update,
        time_sample,
        time_single_step,
    )
...
def train_function(n_epo: int, lr: float = 7e-2, reset: bool = False, decay_rate: float = 0.999):
    ...
    for i in pbar:
        ...
        time_start = time.time()
        (mean_energy, std_energy), (time_update, time_sample, time_single_step) = single_step_train(
            i, current_lr, reset
        )
        ...
        if mean_energy is None:
            logger.warning(
                f"Training stopped early at epoch {i} due to failure in `single_step_train`."
            )
            break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train and profile the NQS Solver using Monte Carlo methods."
    )
    parser.add_argument(
        "--backend",
        type=str,
        default="jax",
        choices=["jax", "numpy"],
        help="Backend to use (jax or numpy)",
    )
    ...
    parser.add_argument(
        "--n_hidden", type=int, default=16, help="Number of hidden units in the RBM"
    )
    parser.add_argument(
        "--numchains", type=int, default=100, help="Number of chains for the sampler"
    )
    parser.add_argument(
        "--numsamples", type=int, default=100, help="Number of samples for the sampler"
    )
    parser.add_argument(
        "--sweep_steps", type=int, default=8, help="Number of sweep steps for the sampler"
    )
    parser.add_argument(
        "--therm_steps", type=int, default=25, help="Number of thermalization steps for the sampler"
    )
    ...
    parser.add_argument(
        ...
        default="scipy_cg",
        choices=["scipy_cg", "cg"],
        help="Solver for the stochastic reconfiguration",
    )
    parser.add_argument(
        "--precond",
        type=str,
        ...
        help="Preconditioner for the stochastic reconfiguration",
    )
    parser.add_argument(
        "--reg",
        type=float,
        default=1,
        help="Regularization parameter for the stochastic reconfiguration",
    )
    parser.add_argument(
        "--maxiter",
        type=int,
        default=500,
        help="Maximum iterations for the stochastic reconfiguration",
    )
    parser.add_argument(
        "--tol", type=float, default=1e-8, help="Tolerance for the stochastic reconfiguration"
    )
    parser.add_argument(
        "--min_sr", type=bool, default=False, help="Use minimum stochastic reconfiguration"
    )
    #! Optional arguments for Profiling
    parser.add_argument("--profile", action="store_true", help="Enable profiling with cProfile")
    parser.add_argument(
        "--output", type=str, default="profile_output.txt", help="Output file for profiling results"
    )
    ...
    precond_id = (
        preconditioners.PreconditionersTypeSym.JACOBI
        if precond_id == "jacobi"
        else preconditioners.PreconditionersTypeSym.IDENTITY
    )
    reg = args.reg
    ...
    backend_np, (rng, rng_k), backend_sp = (
        be_modules if isinstance(be_modules, tuple) else (be_modules, (None, None), None)
    )
    ...
    ham = TransverseFieldIsing(
        lattice=lattice, hilbert_space=None, j=j, hz=h, hx=g, dtype=ham_dtype, backend=backend_np
    )
    ...
    sr_options = SR.SRParams(min_sr=min_sr, maxiter=maxiter, tol=tol, reg=reg, solver_form_s=False)
    ...
    solver_linalg_func = solver_linalg.get_solver_func(
        backend_module=backend_np, use_matvec=False, use_matrix=False, use_fisher=True, sigma=None
    )
    ...
    Plotter.plot(
        ax[0], x=np.arange(len(energies)), y=np.real(energies), marker="o", markersize=0.5, lw=1
    )
