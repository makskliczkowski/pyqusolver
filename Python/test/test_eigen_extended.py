...
def test_scipy_eig_dense_general():
    ...
    rn      = np.linalg.norm(A @ res.eigenvectors - res.eigenvectors @ np.diag(res.eigenvalues)) / (np.linalg.norm(A) + 1e-12)
    assert rn < 1e-6
...
def test_scipy_eigs_sparse_k():
    ...
    k       = 6
    res     = choose_eigensolver('scipy-eigs', A, k=k, hermitian=False, which='LM', tol=1e-6, max_iter=200)
