...
from QES.NQS.src.nqs_train import NQS, NQSTrainer, TDVPAutoTuner
...
class DummyNQS(NQS):
    def __init__(self):
        ...
        self.wrap_single_step_jax = MagicMock(
            return_value=lambda *args, **kwargs: (
                np.zeros(10),
                0.0,
                (MagicMock(mean_energy=1.0, std_energy=0.1, theta0_dot=None), ([], [], False)),
            )
        )
        self.get_params = MagicMock(return_value=np.zeros(10))
        self.set_params = MagicMock()
        self.sample = MagicMock(
            return_value=((None, None), (np.zeros((10, 10)), np.zeros(10)), np.zeros(10))
        )
...
def test_trainer_step_autotuner_logic():
    ...
    trainer.ode_solver.step.return_value = (
        np.zeros(10),
        0.1,
        (
            MagicMock(mean_energy=1.0, std_energy=0.1, sr_converged=True, theta0_dot=None),
            ([], [], False),
        ),
    )
