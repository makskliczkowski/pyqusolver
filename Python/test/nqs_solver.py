...
try:
    ...
    from QES.Solver.MonteCarlo.sampler import JAX_AVAILABLE, Sampler, VMCSampler, get_backend

except ImportError:
    print(
        "Error: Could not import QES modules. Ensure the QES package is installed or the path is correct."
    )
...
def generate_samples(smplr, ntimes=1):
    ...
    logger.info(
        f"Time taken for each sampling: {(time_all - time_sample_f) / ntimes:.2e} seconds.", lvl=3
    )
...
def test_network_quickness(net, backend, ns, ntimes=1):
    ...
    logger.info(
        f"Time taken for each application: {(time_all - time_f) / ntimes:.2e} seconds.", lvl=3
    )
...
def _apply_hamiltonian(
    backend, ham, states, batch_size, configs, configs_ansatze, probabilities, net
):
    """
...
def test_hamiltonian_quickness(
    backend, ham, states, batch_size, configs, configs_ansatze, probabilities, net, ntimes=1
):
    """
    ...
    for _ in range(ntimes - 1):
        r, m, s = _apply_hamiltonian(
            backend, ham, states.copy(), batch_size, configs, configs_ansatze, probabilities, net
        )
    time_all = time.perf_counter() - time_s
    logger.info(f"Time taken for all applications: {time_all:.4f} seconds.", lvl=3)
    logger.info(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="jax",
        choices=["numpy", "jax"],
        help="Backend to use (numpy or jax)",
    )
    ...
    parser.add_argument(
        ...
        help="Data type for the backend",
    )
    ...
    sampler = VMCSampler(
        net=net,  # set the variational model representation
    ...
    (states, logprobas), (configs, configs_ansatze), probabilities = generate_samples(
        sampler, ntimes=args.ntimes
    )
    ...
    logger.info("Applying the Hamiltonian...", lvl=0)
    time_ham_s = time.perf_counter()
    end_time_ham = test_hamiltonian_quickness(
        backend_str,
        ham,
        states,
        batch_size,
    ...
    logger.info("Evaluating through solver...", lvl=0)
