...
from QES.Algebra.globals import u1_sym
...
class TestKitaevSymmetries:
    ...
    def test_kitaev_translation_all_k_sectors(self, small_honeycomb):
        ...
        logger.info(f"Testing translation symmetry on honeycomb lattice with {L} sites", color='red')
        logger.info(f"{'='*80}", color='red')
        ...
        for kx in range(small_honeycomb.lx):
            for ky in range(max(small_honeycomb.ly, 1)):
                ...
                logger.info(f"kx={kx}, ky={ky}: dim={h_k.Nh}, n_eigs={len(E_k)}, GS energy={E_k[0]:.6f}", lvl=1, color='green')

        # Sort and compare
        E_all_k_sorted  = np.sort(E_all_k)

        # Ensure we collected the same number of states as the full Hilbert space
        assert len(E_all_k_sorted) == len(E_full_sorted), (f"Collected states ({len(E_all_k_sorted)}) != full space ({len(E_full_sorted)})")

        max_error       = np.max(np.abs(E_all_k_sorted - E_full_sorted))
        logger.info(f"Translation spectrum reconstruction max error: {max_error:.2e}", lvl=1, color='blue')
        logger.info(f"Hilbert space: {len(E_full_sorted)} -> {total_states} states collected", lvl=1, color='blue')
        logger.info(f"Reduction factor: ~{len(E_full_sorted) / total_states:.1f}x", lvl=1, color='blue')
    ...
    def test_kitaev_translation_k0_sector(self, small_honeycomb):
        ...
        logger.info(f"k=0 sector: {h_k0.Nh} states, GS energy = {np.real(E_k0[np.argmin(np.real(E_k0))]):.6f}", color='red')
        logger.info(f"Max imaginary part of eigenvalues: {imag_part:.2e}", color='red')
        logger.info(f"Energy range: [{np.min(np.real(E_k0)):.4f}, {np.max(np.real(E_k0)):.4f}]", color='red')
    ...
    def test_kitaev_isotropic(self, small_honeycomb):
        ...
        logger.info(f"Isotropic Kitaev (Kx=Ky=Kz=1.0):", color='red')
    ...
    def test_kitaev_ising_x(self, small_honeycomb):
        ...
        E   = np.linalg.eigvalsh(H)
        logger.info(f"Ising-X limit (Kx=1.0, Ky=Kz=0.0):", color='red')
        logger.info(f"  GS energy: {E[0]:.6f}", color='red')
    ...
    def test_kitaev_with_heisenberg(self, small_honeycomb):
        ...
        E = np.linalg.eigvalsh(H)
        print(f"Kitaev-Heisenberg (K=1.0, J=0.5):")
