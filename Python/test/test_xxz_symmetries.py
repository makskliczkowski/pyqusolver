...
class TestXXZSymmetries:
    ...
    def test_xxz_translation_all_k_sectors(self, small_chain):
        ...
        xxz_full = XXZ(lattice=small_chain, hilbert_space=h_full,
        ...
        for k in range(L):
            ...
            xxz_k = XXZ(lattice=small_chain, hilbert_space=h_k,
                       jxy=1.0, jz=0.5, hx=0.0, hz=0.0)
        ...
        assert len(E_all_sorted) == len(E_full_sorted), \
    ...
    def test_xxz_translation_k0_sector(self, small_chain):
        ...
        xxz = XXZ(lattice=small_chain, hilbert_space=h_k0,
                 jxy=1.0, jz=0.5, hx=0.0, hz=0.0)
    ...
    def test_xxz_parity_z_even_odd(self, small_chain):
        ...
        xxz_even = XXZ(lattice=small_chain, hilbert_space=h_parity_even,
        ...
        xxz_odd = XXZ(lattice=small_chain, hilbert_space=h_parity_odd,
                     jxy=1.0, jz=0.5, hx=0.0, hz=0.0)
    ...
    def test_xxz_u1_particle_conservation(self, small_chain):
        ...
        for N in range(L + 1):
            ...
            xxz_N = XXZ(lattice=small_chain, hilbert_space=h_N,
                       jxy=1.0, jz=0.5, hx=0.0, hz=0.0)
    ...
    def test_xxz_translation_plus_parity(self, small_chain):
        ...
        for k in range(L):
            for parity in [1, -1]:
                h_kp = HilbertSpace(lattice=small_chain, 
                                   sym_gen={'translation': k, 'parity': parity})
                xxz_kp = XXZ(lattice=small_chain, hilbert_space=h_kp,
                            jxy=1.0, jz=0.5, hx=0.0, hz=0.0)
        ...
        print(f"Hilbert space reduction: {len(E_full_sorted)} -> {len(E_all_sorted)} "
    ...
    def test_xxz_translation_plus_u1(self, small_chain):
        ...
        for N in range(L + 1):
            u1_sym = get_u1_sym(lat=small_chain, val=N)
            for k in range(L):
                h_kN = HilbertSpace(lattice=small_chain,
                ...
                xxz_kN = XXZ(lattice=small_chain, hilbert_space=h_kN,
                            jxy=1.0, jz=0.5, hx=0.0, hz=0.0)
    ...
    def test_xxz_heisenberg_limit(self, small_chain):
        ...
        h_k0 = HilbertSpace(lattice=small_chain, sym_gen={'translation': 0})
        xxz = XXZ(lattice=small_chain, hilbert_space=h_k0,
                 jxy=1.0, delta=1.0, hx=0.0, hz=0.0)  # Delta=1 => XXX
    ...
    def test_xxz_xy_limit(self, small_chain):
        ...
        h_k0 = HilbertSpace(lattice=small_chain, sym_gen={'translation': 0})
        xxz = XXZ(lattice=small_chain, hilbert_space=h_k0,
                 jxy=1.0, delta=0.0, hx=0.0, hz=0.0)  # Delta=0 => XY
    ...
    def test_xxz_ising_limit(self, small_chain):
        ...
        h_k0 = HilbertSpace(lattice=small_chain, sym_gen={'translation': 0})
        xxz = XXZ(lattice=small_chain, hilbert_space=h_k0,
                 jxy=0.0, jz=1.0, hx=0.0, hz=0.0)  # Jxy=0 => Ising
