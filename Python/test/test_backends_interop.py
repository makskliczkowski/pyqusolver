...
try:
    from QES.Algebra.backends import get_available_backends, get_backend, get_backend_registry
...
class TestMatrixConstruction:
    ...
    def test_from_hermitian_matrix(self):
        """Test creating QuadraticHamiltonian from Hermitian matrix."""
        h_matrix = np.array(
            [[0, -1, 0, 0], [-1, 0, -1, 0], [0, -1, 0, -1], [0, 0, -1, 0]], dtype=complex
        )
    ...
    def test_from_bdg_matrices(self):
        """Test creating QuadraticHamiltonian from BdG matrices."""
        h_matrix_bdg = np.array([[0, 1], [1, 0]], dtype=complex)

        v_matrix_bdg = np.array([[0, 0.5], [0.5, 0]], dtype=complex)
