...
def trace_one_element(ns=6, k_in=0, k_out=0, i_full=0, j_full=1):
    ...
    hil_in = HilbertSpace(
        lattice=lattice, sym_gen=[(SymmetryGenerators.Translation_x, k_in)], gen_mapping=True
    )
    hil_out = HilbertSpace(
        lattice=lattice, sym_gen=[(SymmetryGenerators.Translation_x, k_out)], gen_mapping=True
    )
    ...
    print("mapping_in[:20]=", hil_in.mapping[:20])
    print("normalization_in[:20]=", hil_in.normalization[:20])
    print("\nHilbert out mapping and normalization (first 20):")
    print("mapping_out[:20]=", hil_out.mapping[:20])
    print("normalization_out[:20]=", hil_out.normalization[:20])
    ...
    print(
        "\nContributions to H_rec[{},{}] from sector pair k_out={},k_in={}:".format(
            i_full, j_full, k_out, k_in
        )
    )
    total = 0
