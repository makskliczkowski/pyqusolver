...
def get_entanglement_entropy(hamil: HeisenbergKitaev, hilbert: HilbertSpace, subsystem: Union[int, float]) -> Tuple[List[float], Dict[Tuple[int, int], float], List[float], Dict[str, float]]:
    """
    ...
    """
    from QES.Algebra.Symmetries.jit.density_jit         import rho_symmetries, mutual_information_symmetries, topological_entropy_symmetries
    ...
    for state_idx in range(num_states):
        ...
        try:
            ...
            logger.error(f"Failed to compute Schmidt decomposition for state {state_idx}: {e}", lvl=0)
            entropies.append(np.nan)
            continue
        S_ent   = entropy(vals, q=1)
        entropies.append(S_ent)
    
    logger.info(f"Computed entanglement entropy for {num_states} states on subsystem of size {va} sites.", color='green', lvl=1)
    
    # 2. Compute Mutual Information for the Ground State (state 0)
    psi_gs = hamil.eig_vec[:, 0]
    
    if has_sym:
        logger.info(f"Computing mutual information with symmetries for all site pairs.", color='green', lvl=1)
        try:
            for i in range(ns):
        ...
        logger.info(f"Computing mutual information for all site pairs.", color='green', lvl=1)
        for i in range(ns):
            for j in range(i+1, ns):
                ...
                try:
                    ...
                    logger.error(f"Failed to compute mutual information for sites ({i}, {j}): {e}", lvl=0)
    
    logger.info(f"Successfully computed mutual information for all site pairs.", color='green', lvl=1)
    ...
    if ns >= 12: # TEE typically requires larger systems for meaningful results
        try:
            ...
            if has_sym:
                ...
                tee_results = ent_mod.topological_entropy(state=psi_gs, regions=regions, construction='kitaev_preskill')
                gamma = tee_results['gamma']
                topological = tee_results
                
            logger.info(f"Computed topological entanglement entropy: γ = {gamma:.6f}", color='green', lvl=1)
        except Exception as e:
