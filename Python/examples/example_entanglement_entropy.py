...
def get_entanglement_entropy_fixed(
    hamil, hilbert, subsystem: Union[int, float]
) -> Tuple[List[float], Dict, List[float], List[float], Dict]:
    ...
    if logger:
        logger.info(
            f"Computed entanglement entropy for {num_states} states (va={va} sites)",
            color="green",
            lvl=1,
        )
    ...
    try:
        ...
        if ns >= min_system_size:
            ...
            regions_topo = hamil.lattice.regions.region_kitaev_preskill(
                origin=None, radius=target_radius  # Use center
            )

            # Verify regions are non-trivial
            total_sites_in_ABC = len(
                set(regions_topo["A"]) | set(regions_topo["B"]) | set(regions_topo["C"])
            )

            if total_sites_in_ABC >= ns:
                if logger:
                    logger.warning(
                        f"Regions A∪B∪C cover entire system ({total_sites_in_ABC}/{ns} sites). TEE will be unreliable.",
                        lvl=1,
                    )

            if logger:
                logger.info(
                    f"TEE regions: |A|={len(regions_topo['A'])}, |B|={len(regions_topo['B'])}, "
                    f"|C|={len(regions_topo['C'])}, |ABC|={total_sites_in_ABC}/{ns}",
                    lvl=2,
                )
            ...
            for state_idx in range(states_to_calc_tee):
                ...
                tee_result = topological_entropy(
                    state=psi_cont, regions=regions_topo, hilbert=hilbert, q=1.0  # Von Neumann
                )
                ...
                if logger and state_idx == 0:
                    ...
                    logger.info(
                        f"Ground state TEE: γ = {gamma:.6f} (expected ≈ 0.693 for toric code)",
                        color="cyan",
                        lvl=2,
                    )
        else:
            if logger:
                logger.warning(
                    f"System too small (ns={ns}) for reliable TEE calculation. Skipping.", lvl=1
                )
    ...
    try:
        ...
        for state_idx in range(states_to_calc_mi):
            ...
            for i in range(min(ns, 9)):  # Limit to first 9 sites for large systems
                for j in range(i + 1, min(ns, 9)):
                    info = mutual_information(state=psi, i=i, j=j, hilbert=hilbert, q=1.0)
        ...
        if logger:
            logger.info(
                f"Computed mutual information for {states_to_calc_mi} states", color="green", lvl=1
            )
    ...
    # ============================================================================
    gamma_values = (
        np.array([t.get("gamma", np.nan) for t in topological]) if topological else np.array([])
    )
...
def check_region_validity(lattice, regions: Dict[str, List[int]], logger=None):
    ...
    if overlap_AB or overlap_BC or overlap_AC:
        if logger:
            logger.error(
                f"Regions have overlaps: AB={len(overlap_AB)}, BC={len(overlap_BC)}, AC={len(overlap_AC)}",
                lvl=0,
            )
        return False
    ...
    if total >= ns:
        if logger:
            logger.warning(
                f"Regions cover entire system ({total}/{ns} sites). TEE will be 0 for pure states!",
                lvl=1,
            )
        return False
    ...
    if fraction < 0.3 or fraction > 0.7:
        if logger:
            logger.warning(
                f"Regions cover {fraction*100:.1f}% of system. Optimal range is 30-70%.", lvl=1
            )
    ...
    if max_size > 3 * min_size:
        if logger:
            logger.warning(
                f"Region sizes are imbalanced: A={len(A)}, B={len(B)}, C={len(C)}", lvl=1
            )

    if logger:
        logger.info(
            f"✓ Regions valid: |A|={len(A)}, |B|={len(B)}, |C|={len(C)}, |ABC|={total}/{ns} ({fraction*100:.1f}%)",
            color="green",
            lvl=2,
        )
