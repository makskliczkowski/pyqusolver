...
class NQSLoss(EvaluationResult):
    ...
    @property
    def local_loss(self) -> Array:
        return self.values

    @local_loss.setter
    def local_loss(self, v: Array):
        self.values = v
...
class NQSEvalEngine:
    ...
    def __init__(
        self, nqs: "NQS", batch_size: Optional[int] = None, evaluate_stats: bool = True, **kwargs
    ):
        """
    ...
    def ansatz(
        ...
        *,
        ...
        if return_stats is None:
            return_stats = self.evaluate_stats
        ...
        if states is None or len(states) == 0:
            (_, _), (states, ansatze), (_) = self.nqs.sample(
            ...
            return EvaluationResult(
                values=ansatze, has_stats=return_stats, backend_used=self.nqs.backend_str
            )
        ...
        if return_values:
            return vals
        return EvaluationResult(
            values=vals, has_stats=return_stats, backend_used=self.nqs.backend_str
        )
        ...
        *,
        ...
        try:
            ...
            for idx, vals in enumerate(values):
                ...
                mean_val = (
                    np.asarray(means[idx]).item()
                    if np.ndim(means[idx]) <= 1
                    else np.asarray(means[idx])
                )
                std_val = (
                    np.asarray(stds[idx]).item()
                    if np.ndim(stds[idx]) <= 1
                    else np.asarray(stds[idx])
                )
