...
class TDVPAutoTuner:
    ...
    def update(self,
               ...
               current_energy: float) -> Tuple[Dict[str, Any], bool, List[str]]:
        """
        ...
        if not sr_converged:
            ...
            # If SR fails, we probably shouldn't trust the step fully, but maybe acceptable if residual small
            # For now, just increase shift for next time
        else:
            # If converged fine, try to relax regularization slowly
        ...
        # 3. Learning Rate / Step Size (Energy Stability)
        # -----------------------------------------------
