...
class NQSSymmetricAnsatz:
    ...
    def set(self, use_symmetries: bool = True) -> None:
        ...
        if hilbert is None:
            self._nqs.log(
                "Symmetry projection skipped: No Hilbert space provided to NQS.",
                lvl=1,
                color="yellow",
            )
            self.unset()
        ...
        sym_container = getattr(hilbert, "sym_container", None) or getattr(
            hilbert, "_sym_container", None
        )
        if sym_container is None or len(sym_container.generators) == 0:
            self._nqs.log(
                "Symmetry projection skipped: No symmetry generators found in Hilbert space.",
                lvl=1,
                color="yellow",
            )
            self.unset()
        ...
        try:
            ...
            self._nqs.log(
                f"Symmetry projection enabled for {len(sym_container.generators)} generators: {self._nqs._model.sym}",
                lvl=1,
                color="green",
            )
