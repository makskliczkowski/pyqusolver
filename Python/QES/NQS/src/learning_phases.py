...
class PhaseType(Enum):
    ...
    PRE_TRAINING    = auto()    # Pre-training phase
    MAIN            = auto()    # Main optimization phase
    REFINEMENT      = auto()    # Refinement phase
    CUSTOM          = auto()    # Custom user-defined phase
...
class LearningPhase:
    ...
    -----------
        name (str): 
            Descriptive name of the phase (e.g., "pre-training", "main")
        epochs (int): 
            Number of training epochs in this phase
        ...
        lr_decay (float): 
            Learning rate decay factor per epoch (multiplicative)
        lr_min (float): 
            Minimum learning rate (clamp lower bound)
        ---
        regularization (float): 
            Regularization strength for this phase
        reg_schedule (str): 
            Regularization schedule ('constant', 'exponential', 'linear', 'adaptive')
        ---
        loss_type (str): 
            Type of loss function to use ('energy', 'variance', 'combined', 'custom')
        loss_weight_energy (float): 
            Weight of energy term in combined loss
        loss_weight_variance (float): 
            Weight of variance term in combined loss
        ---
        beta_penalty (float): 
            Beta penalty for excited state orthogonality
        ---
        on_phase_start (Optional[Callable]):    
            Callback when phase begins
    ...
    def __post_init__(self):
        ...
        if self.epochs <= 0:                        raise ValueError(f"epochs must be positive, got {self.epochs}")
        if self.learning_rate <= 0:                 raise ValueError(f"learning_rate must be positive, got {self.learning_rate}")
        if self.lr_min <= 0:                        raise ValueError(f"lr_min must be positive, got {self.lr_min}")
        if self.lr_decay <= 0 or self.lr_decay > 1: raise ValueError(f"lr_decay must be in (0, 1], got {self.lr_decay}")
        if self.regularization < 0:                 raise ValueError(f"regularization must be non-negative, got {self.regularization}")
    ...
    def get_regularization(self, epoch: int) -> float:
        ...
        if self.reg_schedule == 'constant':
            ...
            factor = epoch / max(self.epochs - 1, 1)        # Avoid division by zero
...
PRESETS = {
    "default"   : [
        DEFAULT_PRE_TRAINING, DEFAULT_MAIN, DEFAULT_REFINEMENT
    ],
...
PRESETS['frustrated']   = PRESETS['kitaev']
PRESETS['quick']        = PRESETS['fast']
PRESETS['test']         = PRESETS['fast']
...
class LearningPhaseScheduler:
    ...
    def on_phase_start(self):
        ...
        phase = self.current_phase
        self.logger(f"Starting {phase.phase_type.name} phase: '{phase.name}' " + f"({phase.epochs} epochs, lr={phase.learning_rate:.2e})", color='green')
    ...
    def on_phase_end(self):
        ...
        phase = self.current_phase
        self.logger(f"Completed {phase.phase_type.name} phase: '{phase.name}'", color='cyan')
...
# -----------------------------------
#! Preset learning phase creation
# -----------------------------------
