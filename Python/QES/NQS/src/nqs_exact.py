    ...
    if nqs_instance._nqsproblem.typ == 'wavefunction':
        ...
        if not stats.has_exact:
            if nqs_instance.model.eig_val is None:
                nqs_instance.model.diagonalize( method              =   'lanczos', 
                                        k                           =   kwargs.get('k', 6), 
                                        store_basis                 =   False, 
            ...
            # Log results
            if nstate == 0:
                logfun(f"Exact ground state energy: {nqs_instance.model.eig_val[0]:.6f}",           lvl=1, color='green')
            else:
                logfun(f"Exact state[{nstate}] energy: {nqs_instance.model.eig_val[nstate]:.6f}",   lvl=1, color='green')
                logfun(f"Lowest energies: {nqs_instance.model.eig_val[:max(nstate, 5)]}",           lvl=2, color='green')
...
def load_exact_impl(nqs_instance: 'NQS', filepath: str, *, key: str = 'energy_values'):
    ...
    nqs_instance.exact = {
                            ...
                            'exact_energy'      : float(exact_values) if np.ndim(exact_values) == 0 else exact_values[nqs_instance._nthstate]
                        }
...
def set_exact_impl(nqs_instance: 'NQS', info: Optional[dict]):
    ...
    if isinstance(info, dict):
        ...
        if info is not None: logfun(f"Exact information set: {info.get('exact_predictions', [None] * (nqs_instance._nthstate + 1))[nqs_instance._nthstate]}", lvl=1)
        ...
        logfun(f"Exact information set from array: {nqs_instance._exact_info['exact_energy']}", lvl=1)
    else:
