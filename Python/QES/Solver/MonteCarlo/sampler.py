...
class Sampler(ABC):
    ...
    def num_rejected(self):
        return self.num_proposed - self.num_accepted
            ...
            try:
                ...
                if self._hilbert is None or True:
                    if different_initials:
                        ...
                        self._initstate = initialize_state(
                            initstate,
                            modes=modes_val,
                            hilbert=self._hilbert,
                            shape=self._shape,
    ...
    def set_chains(
        self, initstate: Union[np.ndarray, jnp.ndarray], numchains: Optional[int] = None
    ):
        """
...
def get_update_function(rule: Union[str, UpdateRule], backend="jax", **kwargs) -> Callable:
    ...
    if rule == UpdateRule.LOCAL:
        ...
        if hilbert.local_space is not None and LocalSpaceTypes is not None:
            ...
            if hilbert.local_space.typ not in compatible_types:
                ...
                warnings.warn(
                    f"EXCHANGE update rule requested for local space type '{hilbert.local_space.typ}', "
    ...
    ###############################################################
        ...
        if isinstance(patterns, str):
            ...
            if hilbert is None or hilbert.lattice is None:
                raise ValueError(
                    f"Hilbert space with lattice is required for '{patterns}' global updates."
                )
