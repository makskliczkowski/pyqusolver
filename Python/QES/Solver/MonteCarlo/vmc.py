...
if JAX_AVAILABLE:
    ...
    class JaxVMCSampler(Sampler):
        """
        High-Performance JAX Sampler using JIT compilation and Iterative Key Splitting.
        """
        def __init__(self, net, shape, rng_k, mu=2.0, beta=1.0, 
                    therm_steps=100, sweep_steps=100, num_chains=10, 
                    initstate='RND', **kwargs):
            
            super().__init__(shape, num_chains, kwargs.get('numsamples', 1000), initstate, seed=None, backend='jax', **kwargs)
            
            self._rng_k         = rng_k
            self._net_callable  = self._resolve_net(net)
            ...
            # Stats Counters (JAX Arrays)
            self._num_proposed  = jnp.zeros(num_chains, dtype=DEFAULT_JP_INT_TYPE)
            self._num_accepted  = jnp.zeros(num_chains, dtype=DEFAULT_JP_INT_TYPE)            

            # Internal state
            self._states        = self._initialize_states(initstate, num_chains, shape, rng_k)
            self._log_probs     = None
            
            # Compilation Cache
            self._kernel        = jax.jit(self._make_kernel(mu, beta, sweep_steps))

        def _resolve_net(self, net):
            if hasattr(net, 'apply'):       return net.apply
            if hasattr(net, 'get_apply'):   return net.get_apply()
            if hasattr(net, '__call__'):    return net.__call__
            if callable(net):               return net
            raise ValueError("Net must be callable or Flax module.")

        def reset(self):
            # Generate initial states
            # We use a subkey for init generation to not mess up main RNG flow
            k1, self._rng_k     = jax.random.split(self._rng_k)
            
            # Use the helper function with backend='jax'
            # Note: _state_distinguish handles broadcasting to num_chains if needed
            # But usually it returns a single state, so we stack.
            single_state        = _state_distinguish(self._initstate_spec, self._modes, None, self._shape, 
                                self._mode_repr, rng_k=k1, backend='jax')
            
            # Stack to (num_chains, ...)
            self._states = jnp.stack([single_state] * self._num_chains, axis=0)
            
            self._log_probs = None
            self._num_proposed = jnp.zeros(self._num_chains, dtype=DEFAULT_JP_INT_TYPE)
            self._num_accepted = jnp.zeros(self._num_chains, dtype=DEFAULT_JP_INT_TYPE)

        # ----------------------------------------------------------------------
        # The JAX Kernel (Iterative Keys + Scan)
        # ----------------------------------------------------------------------
        def _make_kernel(self, mu, beta, steps_per_sweep):
            
            def _log_prob(p, x): 
                return jnp.real(self._net_callable(p, x))

            def _kernel_func(carry, _):
                # Unpack: State, LogProb, Key, Stats
                chain, log_psi, key, n_prop, n_acc = carry
                
                # 1. Iterative Key Splitting (Memory Efficient)
                key_step, key_next = jax.random.split(key)
                key_flip, key_acc = jax.random.split(key_step)
                
                # 2. Propose (Local Flip)
                # Select random site to flip for each chain
                n_chains, n_sites = chain.shape
                flip_indices = jax.random.randint(key_flip, (n_chains,), 0, n_sites)
                
                # Custom flip logic (assuming binary 0/1 or -1/1)
                # Efficient JAX flip without if/else
                # Assuming 0/1: new = 1 - old. Assuming -1/1: new = -old
                chain_prop = chain.at[jnp.arange(n_chains), flip_indices].set(
                    1.0 - chain[jnp.arange(n_chains), flip_indices]
                )
                
                # 3. Evaluate
                log_psi_prop = _log_prob(self._params_frozen, chain_prop) # Use frozen params closure
                
                # 4. Metropolis Ratio
                # log_acc = beta * mu * Re(log_psi_new - log_psi_old)
                delta_log = log_psi_prop - log_psi
                ratio = jnp.exp(beta * mu * delta_log)
                
                # 5. Accept/Reject
                rnd_uni = jax.random.uniform(key_acc, (n_chains,))
                accept_mask = rnd_uni < ratio
                
                # 6. Update
                # broadcast mask for shape (N_chains, N_sites)
                mask_b = accept_mask[:, None]
                chain_new = jnp.where(mask_b, chain_prop, chain)
                log_psi_new = jnp.where(accept_mask, log_psi_prop, log_psi)
                
                # 7. Stats
                new_prop = n_prop + 1
                new_acc = n_acc + accept_mask.astype(jnp.int32)
                
                return (chain_new, log_psi_new, key_next, new_prop, new_acc), chain_new

            return _kernel_func

        def sample(self, params, num_samples=None, **kwargs):
            n_samples = num_samples or self._num_samples
            
            # 1. Store params for the closure (needed for the kernel)
            self._params_frozen = params 
            
            # 2. Initial Log Probs (if not cached)
            if self._log_probs is None:
                self._log_probs = jnp.real(jax.vmap(lambda x: self._net_callable(params, x))(self._states))

            # 3. Setup Carry
            # Initial stats counters
            n_prop = jnp.zeros(self._num_chains, dtype=jnp.int32)
            n_acc = jnp.zeros(self._num_chains, dtype=jnp.int32)
            
            init_carry = (self._states, self._log_probs, self._rng_k, n_prop, n_acc)
            
            # 4. Thermalize (Run without collecting)
            # Use lax.fori_loop or simple scan discarding output
            kernel = self._kernel # Retrieved from cache
            
            # We run therm_steps * sweep_steps
            # To avoid huge compile times, we scan the kernel 'sweep_steps' times inside a python loop
            # or a larger scan.
            
            # 5. Sampling Loop
            # We use lax.scan to collect 'n_samples' configurations
            # Each 'scan step' performs 'sweep_steps' MCMC updates
            
            def scan_step(carry, _):
                # Run MCMC for `sweep_steps`
                # We use lax.scan internally for the sweep to stay in XLA
                final_sweep_carry, _ = jax.lax.scan(kernel, carry, None, length=self._sweep_steps)
                # Collect the state at the end of the sweep
                state_captured = final_sweep_carry[0]
                log_psi_captured = final_sweep_carry[1]
                return final_sweep_carry, (state_captured, log_psi_captured)

            final_carry, (batch_states, batch_log_psi) = jax.lax.scan(
                scan_step, init_carry, None, length=n_samples
            )
            
            # 6. Update internal state
            self._states, self._log_probs, self._rng_k, self._num_proposed, self._num_accepted = final_carry
            
            # 7. Flatten output: (Num_samples, Num_chains, ...) -> (Batch, ...)
            flat_states = batch_states.reshape(-1, *self._shape)
            flat_log_psi = batch_log_psi.reshape(-1)
            
            # 8. Compute Weights (Born Rule -> 1.0, modified -> reweight)
            # Assuming mu=2 (Born), weights are 1.0. If mu!=2, calculate exp(...)
            weights = jnp.ones_like(flat_log_psi) 
            
            return (self._states, self._log_probs), (flat_states, flat_log_psi), weights

# ==============================================================================
# NumPy Implementation (Legacy / CPU Fallback)
# ==============================================================================

class NumpyVMCSampler(Sampler):
    """
    Standard NumPy/Numba implementation.
    Preserves original functionality for CPU-only environments.
    """
    def __init__(self, net, shape, rng, mu=2.0, beta=1.0, 
                 therm_steps=100, sweep_steps=100, num_chains=10, 
                 initstate='RND', logprob_fact=0.5, **kwargs):
        
        super().__init__(shape, num_chains, kwargs.get('numsamples', 1000), 
                         initstate, None, 'numpy', **kwargs)
        
        if rng is None:
            raise ValueError("rng (numpy.random.Generator) must be provided for NumpyVMCSampler")
        ...
        # Stats
        self._num_proposed = np.zeros(num_chains, dtype=DEFAULT_NP_INT_TYPE)
        self._num_accepted = np.zeros(num_chains, dtype=DEFAULT_NP_INT_TYPE)
    ...
    def reset(self):
        # Generate initial states
        # For numpy, we just iterate or use the helper
        single_state = _state_distinguish(self._initstate_spec, self._modes, None, self._shape, 
                                          self._mode_repr, rng=self._rng, backend='numpy')
        
        self._states = np.stack([single_state] * self._num_chains, axis=0)
        self._log_probs = None
        self._num_proposed[:] = 0
        self._num_accepted[:] = 0

    def logprob(self, x, params):
        # Helper to compute logprobs using the net callable
        # Wrapper to handle array of states
        res = []
        for i in range(x.shape[0]):
            res.append(self._net(params, x[i]))
        return np.array(res)

    def sample(self, params, num_samples=None) -> Tuple[Any, Any, Any]:
        """
        Executes the Numba/NumPy logic.
        """
        used_samples = num_samples if num_samples is not None else self._num_samples
        
        # 1. Init Log Probs
        if self._log_probs is None:
            self._log_probs = self.logprob(self._states, params)

        # 2. Thermalize
        # Calls the Numba static function
        self._states, self._log_probs, self._num_proposed, self._num_accepted = \
            _run_mcmc_steps_np(
                chain=self._states,
                logprobas=self._log_probs,
                ...
                log_proba_fun=self.logprob, # Passed as python func, slower but compatible
                accept_config_fun=_acceptance_probability_np,
                net_callable_fun=None # unused if log_proba_fun handles it
            )
        ...
        for _ in range(used_samples):
            # Run one sweep (steps between samples)
            self._states, self._log_probs, self._num_proposed, self._num_accepted = \
                _run_mcmc_steps_np(
                    chain=self._states,
                    logprobas=self._log_probs,
                    ...
                    accept_config_fun=_acceptance_probability_np,
                    net_callable_fun=None
                )
            
            configs_list.append(self._states.copy())
            log_list.append(self._log_probs.copy())

        # 4. Format
        # (Num_samples, Num_chains, ...) -> (Total, ...)
        configs = np.concatenate(configs_list, axis=0) # [N_samples*N_chains, ...]
        configs = configs.reshape(-1, *self._shape)
        
        logs = np.concatenate(log_list, axis=0)
        logs = logs.reshape(-1)
        
        # Weights
        exponent = (1.0 / self._logprob_fact - self._mu)
        weights = np.exp(exponent * np.real(logs))
        norm = np.sum(weights)
        weights = weights / norm * weights.size
        
        return (self._states, self._log_probs), (configs, logs), weights

@numba.njit
def _acceptance_probability_np(current_val, candidate_val, beta, mu):
    # Log-domain acceptance: beta * mu * Re(log_new - log_old)
    log_acc = beta * mu * np.real(candidate_val - current_val)
    return np.exp(log_acc)

def _run_mcmc_steps_np(chain, logprobas, num_proposed, num_accepted, params, rng, steps, mu, beta, update_proposer, log_proba_fun, accept_config_fun, net_callable_fun):
    # This function cannot be fully numba-jitted if log_proba_fun calls a Python network object.
    # However, for structure preservation, we keep the logic here.
    # If the network is simple matrix math, it could be jitted.
    
    current_val = logprobas
    n_chains = chain.shape[0]
    
    for _ in range(steps):
        # 1. Propose
        new_chain = update_proposer(chain.copy()) # Numba proposer handles rng internally via partial
        
        # 2. Evaluate (Likely Python call)
        new_val = log_proba_fun(new_chain, params)
        
        # 3. Ratio
        acc_prob = accept_config_fun(current_val, new_val, beta, mu)
        
        # 4. Accept
        rand_vals = rng.random(n_chains)
        accepted = rand_vals < acc_prob
        
        # 5. Update
        # numpy fancy indexing
        chain[accepted] = new_chain[accepted]
        current_val[accepted] = new_val[accepted]
...
class SamplerType(Enum):
    ...
    """
    MCSampler       = auto()
    ExactSampler    = auto() # Placeholder for future #!TODO: implementation
...
def get_sampler(typek: Union[str, SamplerType, Sampler], *args, **kwargs) -> Sampler:
    ...
    if isinstance(typek, Sampler):              # is already a sampler instance
        return typek
    elif isinstance(type, VMCSampler):           # is a sampler class
        return typek
