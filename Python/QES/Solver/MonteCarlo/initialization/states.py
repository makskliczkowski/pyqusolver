...
from    typing import Optional, Union, Tuple, TYPE_CHECKING, Callable, Dict, Any

try:
    from QES.general_python.algebra.utils       import get_backend, DEFAULT_NP_FLOAT_TYPE
...
def _set_state_rand_fixed(modes         : int                           = 2,
                        hilbert         : Optional['HilbertSpace']      = None,
                        shape           : Union[int, Tuple[int, ...]]   = (1,),
                        mode_repr       : float                         = 0.5,
                        backend         : str                           = 'default',
    ...
    if Binary.BACKEND_DEF_SPIN:
        ...
        arr = np.concatenate([np.full(n_up, mode_repr, dtype=float), np.full(n_down, -mode_repr, dtype=float)])
    else:
        # Binary representation: up = mode_repr (usually 1), down = 0
        arr = np.concatenate([np.full(n_up, mode_repr, dtype=float), np.full(n_down, 0.0, dtype=float)])
...
def initialize_state(statetype,
                    modes       : int,
    ...
    # 1. Direct Array
    if isinstance(statetype, (np.ndarray)) or (JAX_AVAILABLE and isinstance(statetype, jnp.ndarray)):
        return statetype
