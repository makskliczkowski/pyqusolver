...
class SymmetryOperator:
    ...
    symmetry_class: SymmetryClass = (
        SymmetryClass.GENERIC
    )  # Default to GENERIC, override in subclasses
    compatible_with: Set[SymmetryClass] = (
        set()
    )  # Unconditionally compatible symmetries, e.g., PARITY with U1_*
    momentum_dependent: Dict[MomentumSector, Set[SymmetryClass]] = (
        {}
    )  # Compatibility at specific momentum sectors, e.g., TRANSLATION-REFLECTION
    supported_local_spaces: Set[LocalSpaceTypes] = (
        set()
    )  # Empty = universal, e.g., spin-1/2, fermions, bosons, etc.
    ...
    def apply_jax(self, state: "jnp.ndarray", **kwargs) -> Tuple["jnp.ndarray", complex]:
        ...
        **kwargs : dict
        ...
        if not JAX_AVAILABLE:
            raise ImportError(
                f"JAX not available - cannot use apply_jax() in {self.__class__.__name__}"
            )
    ...
    def commutes_with(self, other: "SymmetryOperator", **kwargs) -> bool:
        ...
        **kwargs : dict
        ...
        if self.is_real_sector(**kwargs) and other.is_real_sector(**kwargs):
            ...
            if other.symmetry_class in self.momentum_dependent.get(
                MomentumSector.ZERO, set()
            ) or other.symmetry_class in self.momentum_dependent.get(MomentumSector.PI, set()):
                return True
    ...
    def check_boundary_conditions(
        self, lattice: Optional[Any] = None, **kwargs
    ) -> Tuple[bool, str]:
        """
    ...
    def is_compatible_with_global_symmetry(
        self, global_symmetry: Any, **kwargs
    ) -> Tuple[bool, str]:
        """
