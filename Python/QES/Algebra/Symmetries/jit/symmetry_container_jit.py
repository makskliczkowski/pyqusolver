...
def apply_group_element_compiled(
    state: np.int64,  # input state
...
def apply_group_element_fast(state, ns, g, cg_args, tb_args):
    ...
    return apply_group_element_compiled(
        state,
        ns,
        g,
        n_ops,
...
def _compute_normalization_compiled(
    state: np.int64, ns: np.int64, n_group: np.int64, cg_args: Tuple, tb_args: Tuple
) -> np.float64:
    """
