...
import  numba
import  numpy   as np
from    typing  import TYPE_CHECKING, Tuple, Optional, Union, List, Dict
...
def rho_symmetries(state, va: Union[int, np.ndarray, List[int]], hilbert: 'HilbertSpace', cache_size=256, full_state: Optional[np.ndarray]=None) -> np.ndarray:
    ...
    if hilbert.nhfull <= _NUMBA_OVERHEAD_SIZE or not hilbert.has_sym:
        from QES.general_python.physics.density_matrix import rho_numpy, rho_numba_mask
        if hilbert.has_sym:
