...
def check_compatibility(symmetries      : List[SymmetryOperator],
                        warn_callback   : Optional[callable] = None) -> Tuple[List[SymmetryOperator], Set[str]]:
    """
    ...
    for i, sym_i in enumerate(symmetries):
        ...
        for sym_j in compatible:
            if not sym_i.commutes_with(sym_j, momentum_sector):
                ...
                if momentum_sector is not None: msg += f" at momentum sector {momentum_sector.name}"
                msg            += "; removing."
...
def build_symmetry_group(generators: List[SymmetryOperator], max_group_size: int = 10000) -> List[Tuple[SymmetryOperator, ...]]:
    """
...
def infer_momentum_sector_from_operators(symmetries: List[SymmetryOperator]) -> Optional[MomentumSector]:
    """
