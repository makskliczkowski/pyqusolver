    ...
    def matvec(
        ...
        *args,
        ...
        try:
            ...
            raise ImportError(
                "JIT matrix builder not available. Ensure Numba is installed and QES is properly set up."
            )
        ...
        # Use projected kernel if forced OR if we are changing sectors (assuming we want to use symmetry if available)
        use_projected_kernel = (compact_data_out is not None and compact_data_in is not None) and (
            force_project or not is_same_sector
        )
        # Use fast kernel if same sector and compact data available and not forced to project
        use_fast_kernel = (
        ...
        if not multithreaded:
            ...
            th_buffer = ensure_thread_buffer(
                nh, chunk_size, dtype=dtype, thread_buffers=thread_buffer
            )
            ...
            if use_fast_kernel and nhfull != hilbert_in.nh:
                ...
                _apply_op_batch_compact_jit(
                    vecs_in, vecs_out, op_func, op_args, basis_args, th_buffer, chunk_size
                )
    ...
    def matvec_fourier(
        ...
        *,
        ...
        if not self.type_acting.is_local():
            raise ValueError(
                "Fourier operator application requires a local operator. This means it needs to accept 'i' argument..."
            )
        ...
        if not multithreaded:
            ...
            if use_fast_kernel and (hilbert.nhfull != hilbert.nh):
                ...
                _apply_fourier_batch_compact_seq_jit(
                    vecs_in, vecs_out, phases, self._fun._fun_int, basis_args
                )
            ...
            thread_buffer = ensure_thread_buffer(
                nh, chunk_size, dtype=np.complex128, thread_buffers=thread_buffer
            )
            ...
            if use_fast_kernel and (hilbert.nhfull != hilbert.nh):
                ...
                _apply_fourier_batch_jit(
                    vecs_in, vecs_out, phases, self._fun._fun_int, thread_buffer, chunk_size
                )
