...
try:
    ...
    raise ImportError(
        "QES.Algebra.Operator and QES.Algebra.Hilbert modules are required for spinless fermion operators."
    )
...
if JAX_AVAILABLE:
    from QES.Algebra.Operator.impl.jax.operators_spinless_fermions import (
        c_dag_jnp,
        c_jnp,
        c_k_dag_jnp,
        c_k_jnp,
...
def c_k_np(state: np.ndarray, sites: np.ndarray, k: float, prefactor: float = 1.0):
    ...
    if non_zero == 0:
        return (np.empty((0, ns), dtype=state.dtype), np.empty(0, dtype=_DEFAULT_FLOAT))
    ...
    for i in sites:
        ...
        index += 1
    return ensure_operator_output_shape_numba(
        out_state[:index], out_coeff[:index] / np.sqrt(np.max(index, 1))
    )


@numba.njit
def c_k_dag_int_np(state: int, ns: int, sites: List[int], k: float, prefactor: float = 1.0):
    r"""
    ...
    return (out_state[:index], out_coeff[:index] / np.sqrt(max(index, 1)))


@numba.njit
def c_k_dag_np(state: np.ndarray, sites: List[int], k: float, prefactor: float = 1.0):
    r"""
...
def fermion_composition_integer(is_complex: bool = True, only_apply: bool = False) -> Callable:
    ...
    def fermion_operator_composition_single_op(
    ...
    @numba.njit(nogil=True)
    def fermion_operator_composition_int(
        ...
        for k in range(nops):
            ...
            current_s, current_c, is_diagonal = fermion_operator_composition_single_op(
                state, code, s1, s2, ns
            )
...
def fermion_composition_with_custom(
    is_complex: bool, custom_op_funcs: Dict[int, Callable], custom_op_arity: Dict[int, int]
) -> Callable:
    """
    ...
    @numba.njit(nogil=True)
    def fermion_composition_with_custom_int(
        state: int, nops: int, codes, sites, coeffs, ns: int, out_states, out_vals
    ):
        """
        ...
        for k in range(nops):
            ...
            if code >= CUSTOM_OP_BASE:
                ...
                current_s, current_c, is_diagonal = apply_predefined_fermion_op(
                    state, code, sites[k, 0], sites[k, 1], ns
                )
                handled = True
