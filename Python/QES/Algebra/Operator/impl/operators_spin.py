...
_SIG_0 = np.array([[1, 0], [0, 1]], dtype=float)
_SIG_X = np.array([[0, 1], [1, 0]], dtype=float)
_SIG_Y = np.array([[0, -1j], [1j, 0]], dtype=complex)
_SIG_Z = np.array([[1, 0], [0, -1]], dtype=float)
...
@numba.njit(inline="always")
def sigma_plus_int_np(
    state: int, ns: int, sites: List[int], spin: bool = BACKEND_DEF_SPIN, spin_value: float = _SPIN
):
    r"""
...
@numba.njit(inline="always")
def sigma_plus_np(
    state: np.ndarray, sites: List[int], spin: bool = BACKEND_DEF_SPIN, spin_value: float = _SPIN
):
    r"""
...
@numba.njit(inline="always")
def sigma_pm_int_np(
    state: int, ns: int, sites: List[int], spin: bool = BACKEND_DEF_SPIN, spin_val: float = _SPIN
):
    r"""
...
@numba.njit(inline="always")
def sigma_pm_np(
    state: np.ndarray, sites: List[int], spin: bool = BACKEND_DEF_SPIN, spin_val: float = _SPIN
):
...
def sig_y(
    ...
    """
    is_real = type_act == OperatorTypeActing.Correlation or (
    ...
    for c in op_codes:
        if isinstance(c, str):
            ...
            if c in (0, 1, 2):
                ...
                raise ValueError(
                    f"Invalid Pauli operator integer code: {c}. Must be 0 (X), 1 (Y), or 2 (Z)."
                )
        else:
    ...
    if not return_op:
        ...
        if type_act is None:
            type_act = (
                ...
                if op_sites is not None
                else OperatorTypeActing.Correlation
            )
...
def create_sigma_mixed_jnp(op1_fun: Callable, op2_fun: Callable):
    ...
    def sigma_mixed_jnp(state, sites, spin: bool = BACKEND_DEF_SPIN, spin_value: float = _SPIN):
        ...
        return jax.lax.cond(
            ...
            lambda _: (lambda res: (res[0], res[1].astype(target_dtype)))(
                _apply_second(s_arr1, c_arr1)
            ),
            operand=None,
        )
...
def sig_xy(
    ...
    return make_sigma_mixed(
        "xy", lattice, ns, type_act=type_act, sites=sites, spin_value=spin_value
    )
...
def sigma_composition_integer(
    ...
    def sigma_operator_composition_single_op(state: int, code: int, site1: int, site2: int, ns: int):
        ...
        if code == SPIN_LOOKUP_CODES.sig_x_local:
            ...
            if is_complex:
                ...
                # Y operator is complex, cannot be used in real mode
                current_s = state
                current_c = dtype(0.0)
        ...
        #! 3) CORRELATION OPERATORS
            ...
            current_s = s_arr
            current_c = dtype(c_arr) if is_complex else dtype(c_arr.real)
    ...
    def sigma_operator_composition_int(
...
def sigma_composition_with_custom(
    is_complex: bool,
    ...
    custom_arity_arr = np.array(
        [custom_op_arity.get(code, 1) for code in custom_codes_arr], dtype=np.int32
    )
    ...
    apply_predefined_operator = sigma_composition_integer(
        is_complex, only_apply=True, spin_value=spin_value
    )

    @numba.njit(nogil=True)
    def sigma_operator_composition_with_custom_int(
        state: int, nops: int, codes, sites, coeffs, ns: int, out_states, out_vals
    ):
        """
        ...
        for k in range(nops):
            ...
            if custom_app:
                ...
                current_s, current_c, is_diagonal = apply_predefined_operator(
                    state, code, sites[k, 0], sites[k, 1], coeff, ns
                )
                handled = True
...
def spin_plaquette(
    ...
    *,
    ...
    return (
        pauli_string(op_codes, op_sites, lattice.ns, return_op=return_op, spin_value=spin_value),
        op_bonds,
    )


def spin_plaquettes(
    list_plaquettes: list,
    lattice: "Lattice",
    *,
    bond_to_op=None,
    return_op: bool = False,
