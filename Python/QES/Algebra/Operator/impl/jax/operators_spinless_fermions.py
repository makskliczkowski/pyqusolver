...
if JAX_AVAILABLE:
    ...
    @jit
    def c_dag_int_jnp(state: jnp.int64, ns: int, sites: jnp.ndarray, pref: float = 1.0):
        pos = ns - 1 - sites
        bits = (state >> pos) & jnp.int64(1)
        any_occ = jnp.any(bits == 1)

        def _abort(_):
            return (jnp.array([state], dtype=_DEFAULT_INT), jnp.array([0.0], dtype=_DEFAULT_FLOAT))
        ...
        def _apply(_):
            ...
            (new_state, coeff), _ = lax.scan(body, init=(state, 1.0), xs=sites)
            n_sites = sites.shape[0]
            coeff *= pref**n_sites
            return (
                jnp.array([new_state], dtype=_DEFAULT_INT),
    ...
    @jit
    def c_int_jnp(state: jnp.int64, ns: int, sites: jnp.ndarray, pref: float = 1.0):
        pos = ns - 1 - sites
        bits = (state >> pos) & jnp.int64(1)
    ...
    # @jit
    def c_dag_jnp(state: jnp.ndarray, sites: jnp.ndarray, pref: float = 1.0):
        """
    ...
    # @jit
    def c_jnp(state: jnp.ndarray, sites: jnp.ndarray, pref: float = 1.0):
        bits = jnp.take(state, sites)
        any_empty = jnp.any(bits == 0)
    ...
    @jit
    def c_k_int_jnp(state: jnp.int64, ns: int, sites: jnp.ndarray, k: float, pref: float = 1.0):
        """
    ...
    @jit
    def c_k_dag_int_jnp(state: jnp.int64, ns: int, sites: jnp.ndarray, k: float, pref: float = 1.0):
        r"""
    ...
    @jit
    def c_k_jnp(state: jnp.ndarray, sites: jnp.ndarray, k: float, pref: float = 1.0):
        """
    ...
    @jit
    def c_k_dag_jnp(state: jnp.ndarray, sites: jnp.ndarray, k: float, pref: float = 1.0):
        r"""
    ...
    @jax.jit
    def n_int_jax(state: int | jnp.ndarray, ns: int, sites: Sequence[int], prefactor: float = 1.0):
        r"""
        ...
        return jnp.asarray([state_arr], dtype=_DEFAULT_INT), jnp.asarray(
            [coeff_val], dtype=_DEFAULT_FLOAT
        )

    # @jax.jit
    def n_jax(state: jnp.ndarray, sites: jnp.ndarray, prefactor: float = 1.0):
        r"""
