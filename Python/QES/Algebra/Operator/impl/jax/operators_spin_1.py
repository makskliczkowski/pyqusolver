...
if JAX_AVAILABLE:
    ...
    _S1_IDENTITY_jnp    = jnp.array([[1, 0, 0],
                                    [0, 1, 0],
                                    [0, 0, 1]], dtype=float)

    _S1_X_jnp           = jnp.array([[0, 1, 0],
                                    [1, 0, 1],
                                    [0, 1, 0]], dtype=float) * _SQRT2_INV_jnp

    _S1_Y_jnp           = jnp.array([[0, -1j, 0],
                                    [1j, 0, -1j],
                                    [0, 1j, 0]], dtype=complex) * _SQRT2_INV_jnp

    _S1_Z_jnp           = jnp.array([[1, 0, 0],
                                    [0, 0, 0],
                                    [0, 0, -1]], dtype=float)

    _S1_PLUS_jnp        = jnp.array([[0, 1, 0],
                                    [0, 0, 1],
                                    [0, 0, 0]], dtype=float) * _SQRT2_jnp

    _S1_MINUS_jnp       = jnp.array([[0, 0, 0],
                                    [1, 0, 0],
                                    [0, 1, 0]], dtype=float) * _SQRT2_jnp

    _S1_Z2_jnp          = jnp.array([[1, 0, 0],
                                    [0, 0, 0],
                                    [0, 0, 1]], dtype=float)
    ...
    def spin1_plus_int_jnp(state,
        ...
        def body(i, carry):
            ...
            def compute_branch(_):
                ...
                new_coeff = lax.cond(
                    valid,
                    lambda _: curr_coeff * sqrt2 * spin_value,
                    lambda _: 0.0,
                    operand=None
                )
    ...
    def spin1_plus_jnp(state,
        ...
        def body_fun(i, state_val):
            ...
            def compute_branch(_):
                ...
                new_coeff = lax.cond(
                    valid,
                    lambda _: coeff_in * sqrt2 * spin_value,
                    lambda _: 0.0,
                    operand=None
                )
...
if JAX_AVAILABLE:
    ...
    def spin1_x_int_jnp(state,
        ...
        if len(sites) == 1:
            ...
            return lax.switch(
                local_s,
                [case_plus1, case_zero, case_minus1, case_invalid],
                operand=None
            )
        else:
            raise ValueError("spin1_x_int_jnp supports single site only. Use composition for multi-site.")
    ...
    def spin1_y_int_jnp(state,
        ...
        if len(sites) == 1:
            ...
            def case_zero(_):  # |0⟩ -> i|+1⟩ - i|-1⟩
                state_up = (state & mask) | (0 << bit_pos)
                state_dn = (state & mask) | (2 << bit_pos)
                return jnp.array([state_up, state_dn]), jnp.array([1j * spin_value, -1j * spin_value])
            ...
            raise ValueError("spin1_y_int_jnp supports single site only. Use composition for multi-site.")
...
if JAX_AVAILABLE:
    ...
    def spin1_pm_int_jnp(state,
        ...
        coeff = lax.cond(
            valid,
            lambda _: 2.0 * spin_value * spin_value,
            lambda _: 0.0,
            operand=None
        )
        ...
        new_state = lax.cond(
            valid,
            lambda _: ((state & mask_i & mask_j) | 
                      (new_local_i << bit_pos_i) | 
                      (new_local_j << bit_pos_j)),
            lambda _: state,
            operand=None
        )
...
if JAX_AVAILABLE:
    ...
    @partial(jax.jit, static_argnums=(1, 2, 3))
    def spin1_squared_int_jnp(state,
                              ns: int,
                              sites: Union[List[int], tuple],
                              spin_value: float = _SPIN_1):
        """
...
if JAX_AVAILABLE:
    ...
    @partial(jax.jit, static_argnums=(1, 2, 3))
    def spin1_z_total_int_jnp(state,
                              ns: int,
                              sites: Union[List[int], tuple],
                              spin_value: float = _SPIN_1):
        """
