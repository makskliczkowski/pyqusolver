...
class BasisAwareOperator(SpecialOperator, ABC):
    ...
    def _infer_and_set_default_basis(self):
        ...
        if self._hilbert_space is not None and hasattr(self._hilbert_space, '_basis_type'):
            ...
            self._log(f"Basis inferred: {default_basis} ({self._basis_metadata.get('system_type', 'unknown')})", lvl=2, color="cyan")
        ...
        **kwargs
        ...
        if handler is None:
            ...
            raise NotImplementedError(
                f"No transformation registered: {from_str} -> {to_str}. "
    ...
    def get_transformation_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'transformed_shape'         : self._operator_transformed.shape if self._operator_transformed is not None else None,
    ...
    def _get_symmetry_from_hilbert(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        if hasattr(self._hilbert_space, '_sym_container') and self._hilbert_space._sym_container is not None:
