...
try:
    from QES.Algebra.Operator.operator          import Operator, OperatorTypeActing, OperatorFunction, SymmetryGenerators
    from QES.Algebra.Hilbert.hilbert_local      import LocalSpaceTypes
    
except ImportError as e:
    raise ImportError("QES modules are required for SpecialOperator. Ensure QES is installed.") from e
...
class SpecialOperator(Operator, ABC):
    ...
    def _handle_system(self, ns : Optional[int], hilbert_space : Optional[HilbertSpace], lattice : Optional['Lattice'], logger : Optional['Logger'], **kwargs):
        ...
        try:
            ...
            raise ImportError("QES HilbertSpace module is required for SpecialOperator. Ensure QES is installed.") from e
        ...
        if hilbert_space is not None:
            ...
            if self._physics_type is None and hasattr(hilbert_space, '_local_space') and hilbert_space._local_space is not None:
                self._physics_type = hilbert_space._local_space.typ
            
            # Check mode consistency
            if self._is_manybody and not hilbert_space._is_many_body:
                raise ValueError(SpecialOperator._ERR_MODE_MISMATCH + " Hamiltonian is many-body but HilbertSpace is quadratic.")
        ...
        if self._physics_type is not None:
            try:
                ...
                raise ImportError("QES HilbertLocal module is required for SpecialOperator. Ensure QES is installed.") from e
        ...
        if self._logger:
            self._log(f"Created HilbertSpace: ns={self._ns}, local_space={local_space_arg or 'default'}", lvl=3, color='green', log='debug')
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._ops_nmod_nosites      : List[List[Tuple]]     = [[] for _ in range(self._ns)]  # Non-modifying, global operators
        self._ops_nmod_sites        : List[List[Tuple]]     = [[] for _ in range(self._ns)]  # Non-modifying, local operators
        self._ops_mod_nosites       : List[List[Tuple]]     = [[] for _ in range(self._ns)]  # Modifying, global operators
        self._ops_mod_sites         : List[List[Tuple]]     = [[] for _ in range(self._ns)]  # Modifying, local operators
        ...
        **kwargs
        ...
        if not compute:
            ...
            if hasattr(op_module, 'compute_correlations'):
                ...
                raise AttributeError("Operator module does not have a 'compute_correlations' attribute.")
    ...
    def add(self, operator: Operator, multiplier: Union[float, complex, int], modifies: bool = False, sites = None):
        ...
        try:
            ...
            raise ImportError("QES Operator module could not be loaded. Ensure QES is properly installed.") from e
    ...
    def setup_instruction_codes(self, physics_type: Optional[str] = None) -> None:
        ...
        self._lookup_codes, self._instr_function    = module.get_codes(is_complex=(self._iscpx or self._dtype == np.complex128))
    ...
    def _create_hybrid_composition(self) -> Optional[Callable]:
        ...
        try:
            return self.operators.get_composition(is_cpx=is_complex, custom_op_funcs=self._custom_op_registry, custom_op_arity=self._custom_op_arity)
        except Exception as e:
            self._log(f"Failed to create hybrid composition function: {e}", lvl=3, color="red", log='error')
    ...
    def _register_custom_operator(self, operator: Operator, sites: Optional[List[int]]) -> int:
        ...
        if hasattr(self, '_log'):
            self._log(f"Registered custom operator '{operator.name}' with code {custom_code}, arity={arity}", lvl=2, log='debug')
    ...
    def add_instruction(self, operator: Operator, coefficient: Union[float, complex], sites: Optional[List[int]] = None) -> None:
        """
