...
class BackendRegistry:
    ...
    def __init__(self):
        ...
        self._manager = get_backend_manager()
    ...
    def get(self, name: str) -> Backend:
        ...
        if name not in ["numpy", "jax"]:
            ...
            raise ValueError(f"Backend '{name}' not registered. " f"Available: {available}")
...
def get_backend(name: str) -> Backend:
    ...
    if name == "numpy":
        ...
        if JAX_AVAILABLE:
            available.append("jax")
        raise ValueError(f"Backend '{name}' not recognized. " f"Available backends: {available}")
