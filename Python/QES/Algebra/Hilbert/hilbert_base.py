...
class BaseHilbertSpace(ABC):
    ...
    # Aliases
    def find_repr(self, state, nb: float = 1.0):
    ...
    def transform_to_reduced_space(self, vec_full: np.ndarray) -> np.ndarray:
        ...
        if not self.has_compact_symmetry_data:
            ...
            if self.nh == self.nhfull:
                ...
                raise NotImplementedError(
                    "Transform without compact symmetry data not implemented."
                )
        ...
        if vec_full.shape[0] != len(repr_map):
            raise ValueError(
                f"Vector dimension {vec_full.shape[0]} does not match full Hilbert space {len(repr_map)}"
            )
