...
try:
    from QES.Algebra.Symmetries.symmetry_container import (
        ...
        _binary_search_representative_list,
    )
...
def _determine_matrix_dtype(
    dtype: np.dtype, hilbert_spaces, operator_func=None, ns=None
) -> np.dtype:
    """
    ...
    if (
        not is_real_dtype
        or has_complex_sym
        or repr_phase_is_complex
        or hilbert_dtype_is_complex
        or has_symmetries
        or operator_returns_complex
    ):
...
def _build_same_sector(
    ...
    return sp.csr_matrix(
        (data[:data_idx], (rows[:data_idx], cols[:data_idx])), shape=(nh, nh), dtype=alloc_dtype
    )


def _build_sector_change(
    hilbert_in: object,
    hilbert_out: object,
    operator_func: Callable,
    sparse: bool,
    ...
    representative_list_in = (
        hilbert_in.representative_list
        if hasattr(hilbert_in, "representative_list")
        else np.arange(nh_in, dtype=np.int64)
    )
    representative_list_out = (
        hilbert_out.representative_list
        if hasattr(hilbert_out, "representative_list")
        else np.arange(nh_out, dtype=np.int64)
    )
    ...
    compact_data_in = getattr(
        hilbert_in, "compact_symmetry_data", None
    )  # Needed for projected build
    use_compact = compact_data_out is not None

    if not use_compact:
        raise ValueError(
            "Compact symmetry data missing for output Hilbert space in sector change build."
        )
    ...
    if check_sym and compact_data_in and compact_data_out:
        ...
        try:
            ...
            raise ImportError(
                "Projected sparse matrix builder requires QES.Algebra.Symmetries.jit.matrix_builder_jit module."
            )
...
)  # nogil for thread safety -> allows multi-threading if needed, stands for "no global interpreter lock"
...
def _build_no_hilbert(
    operator_func: Callable, nh: int, ns: int, sparse: bool, max_local_changes: int, dtype: np.dtype
):
    """
    ...
    if not sparse:
        matrix = np.zeros((nh, nh), dtype=alloc_dtype)
        _fill_dense_kernel(
            matrix, operator_func, nh
        )  # Note: operator_func MUST be jitted for this to work efficiently
        return matrix
    ...
    buffer_size = max(
        1024, estimated_nnz
    )  # Minimum size to prevent excessive resizing on tiny systems
    ...
    while not finished:
        ...
        next_row, next_ptr, finished = _fill_sparse_kernel(
            curr_row, nh, rows, cols, vals, curr_ptr, buffer_size, operator_func
        )
    ...
    *,
    ...
    if hilbert_space is None or hilbert_space.nh == hilbert_space.nhfull:
        ...
        if hilbert_space_out is not None:
            raise ValueError(
                "hilbert_space_out not supported when operating in the full Hilbert space (i.e., when hilbert_space is None or nh == nhfull)."
            )
        ...
        if hilbert_space is None:
            if nh is None or ns is None:
                raise ValueError(
                    "If hilbert_space is None, nh (Hilbert space size) and ns (number of spins) must be provided."
                )
        ...
        return _build_no_hilbert(
            operator_func,
            nh or (hilbert_space.dim if hilbert_space is not None else None),
            ns or (hilbert_space.ns if hilbert_space is not None else None),
            sparse,
            max_local_changes,
            dtype,
        )
    ...
    if hilbert_space_out is None:
        return _build_same_sector(
            hilbert_space, operator_func, sparse, max_local_changes, dtype, check_sym
        )
    else:
...
def ensure_thread_buffer(
    nh: int, chunk_size: int, dtype=np.complex128, thread_buffers: np.ndarray | None = None
):
    """
    ...
    if (
        thread_buffers is None
        or thread_buffers.ndim != 3
        or thread_buffers.shape[0] != n_threads
        or thread_buffers.shape[1] != nh
        or thread_buffers.shape[2] != chunk_size
    ):
    ...
    for b_start in range(0, n_batch, chunk_size):
        ...
        for k in numba.prange(nh):
            ...
            for i in range(len(new_states)):
                ...
                idx = repr_map[
                    new_state
                ]  # Get index in reduced basis -> maps to index of representative_list
                if idx == _INVALID_REPR_IDX_NB:
                    continue
...
def get_symmetry_rotation_matrix(
    hilbert_space: HilbertSpace, dtype=np.complex128, *args, to_dense=False
) -> Union[sp.csr_matrix, np.ndarray]:
    """
    ...
    normalization = (
        hilbert_space.normalization
        if hasattr(hilbert_space, "normalization")
        else np.ones(nh_reduced, dtype=np.float64)
    )
