...
class XXZ(hamil_module.Hamiltonian):
    ...
    def __repr__(self) -> str:
        ...
        parts  += [
            ...
            hamil_module.Hamiltonian.fmt("Δ",    self._delta, prec=prec) if self._delta is not None else "",
            ...
            if self._jxy is not None and self._jz is not None:
                ...
                if not jxy_is_zero:
                    if isinstance(self._jxy, list) and isinstance(self._jz, list):
                        self._delta = [self._jz[i] / self._jxy[i] if abs(self._jxy[i]) > 1e-15 else 0.0 
                        ...
                        jxy_val = self._jxy[0] if hasattr(self._jxy, '__getitem__') else self._jxy
                        self._delta = [jz_val / jxy_val if abs(jxy_val) > 1e-15 else 0.0 for jz_val in self._jz]
                    else:
                        jxy_val = self._jxy[0] if hasattr(self._jxy, '__getitem__') else self._jxy
            ...
            for nn_idx in range(nn_forward_num):
                ...
                if self._jxy is not None:
                    ...
                    if not np.isclose(coupling_xy, 0.0):
                        # Add Sx_i Sx_j term
                        self.add(operator=op_sx_sx_c, multiplier=-coupling_xy, sites=[i, j_neighbor], modifies=True)
                        self._log(f"Adding SxSx between ({i}, {j_neighbor}) with multiplier {-coupling_xy:.3f}", lvl=2, log='debug')
                        
                        # Add Sy_i Sy_j term
                        self.add(operator=op_sy_sy_c, multiplier=-coupling_xy, sites=[i, j_neighbor], modifies=True)
                        self._log(f"Adding SySy between ({i}, {j_neighbor}) with multiplier {-coupling_xy:.3f}", lvl=2, log='debug')
                ...
                if self._jz is not None:
                    ...
                    if not np.isclose(coupling_z, 0.0):
                        self.add(operator=op_sz_sz_c, multiplier=-coupling_z, sites=[i, j_neighbor], modifies=False)
                        self._log(f"Adding SzSz between ({i}, {j_neighbor}) with multiplier {-coupling_z:.3f}", lvl=2, log='debug')
    ...
    def Jxy(self) -> Union[List[float], float]:
        ...
        if self._jxy and isinstance(self._jxy, list) and all(np.isclose(x, self._jxy[0]) for x in self._jxy):
            return self._jxy[0]
        return self._jxy
    ...
    def Jz(self) -> Union[List[float], float]:
        ...
        if self._jz and isinstance(self._jz, list) and all(np.isclose(x, self._jz[0]) for x in self._jz):
            return self._jz[0]
        return self._jz
    ...
    def Delta(self) -> Union[List[float], float]:
        ...
        if self._delta and isinstance(self._delta, list) and all(np.isclose(x, self._delta[0]) for x in self._delta):
    ...
    def hx(self) -> Union[List[float], float]:
        ...
        if self._hx and isinstance(self._hx, list) and all(np.isclose(x, self._hx[0]) for x in self._hx):
            return self._hx[0]
        return self._hx
    ...
    def hz(self) -> Union[List[float], float]:
        ...
        if self._hz and isinstance(self._hz, list) and all(np.isclose(x, self._hz[0]) for x in self._hz):
            return self._hz[0]
        return self._hz
