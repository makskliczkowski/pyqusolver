...
class QSM(hamil_module.Hamiltonian):
    ...
    def __repr__(self):
        ...
        different_alpha = (
            not all(np.isclose(a, self.alphas[0]) for a in self.alphas[1:])
            if len(self.alphas) > 1
            else False
        )
        different_h = (
            not all(np.isclose(h, self.h[0]) for h in self.h[1:]) if len(self.h) > 1 else False
        )
        different_xi = (
            not all(np.isclose(xi, self.xi[0]) for xi in self.xi[1:]) if len(self.xi) > 1 else False
        )
    ...
    def _init_hdot(self):
        ...
        if np.issubdtype(self._dtype, np.complexfloating):
            ...
            hdot = random_matrix(
                (self._dimin, self._dimin), typek=RMT.GOE, backend=self._backend, dtype=self._dtype
            )
    ...
    def init_particles(self):
        ...
        # log information
        self._log(
            "alpha = [{}]".format(", ".join(f"{val:.3f}" for val in self._a)), lvl=1, log="debug"
        )
        self._log("u = [{}]".format(", ".join(f"{val:.3f}" for val in self._u)), lvl=2, log="debug")
        self._log(
            "alpha^u = [{}]".format(", ".join(f"{val:.3f}" for val in self._au)), lvl=2, log="debug"
        )
    ...
    def _hamiltonian(self, use_numpy: bool = False):
        ...
        for i in range(self.nout):
            self._log(
                f"QSM: i={i} -> h={self._h[i]:.3e}, a^u={self._au[i]:.3e}", lvl=3, log="debug"
            )
    ...
    def _set_local_energy_operators(self):
        ...
        op_sz_l = operators_spin_module.sig_z(
            ns=self.ns, type_act=operators_spin_module.OperatorTypeActing.Local
        )
        op_sx_sx_c = operators_spin_module.sig_x(
            ns=self.ns, type_act=operators_spin_module.OperatorTypeActing.Correlation
        )
        ...
        for i in range(self.n, self.ns):
            ...
            self.add(
                op_sx_sx_c,
                modifies=True,
                multiplier=self.g0 * self._au[part_idx],
                sites=[i, self._neidot[part_idx]],
            )
