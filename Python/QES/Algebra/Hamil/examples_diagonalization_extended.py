...
def example_scipy_eigh_generalized():
    ...
    """
    print("\n" + "="*80)
    print("Example 2: Generalized Eigenvalue Problem (A v = \lambda B v)")
    print("="*80)
...
def example_scipy_eigs():
    ...
    for i, lam in enumerate(result.eigenvalues):
        print(f"  \lambda[{i}] = {lam.real:+.6f} + {lam.imag:+.6f}j  (|\lambda| = {abs(lam):.6f})")
...
def example_comparison():
    ...
    for method in methods:
        try:
            ...
            print(f"{method:12s}: \lambda_min = {evals[0]:+.8f}, \lambda_max = {evals[-1]:+.8f}")
...
def example_subset_selection():
    ...
    print(f"\nMatrix size: {n}x{n}")
    print(f"Requested eigenvalues at indices 10-19")
    print(f"Computed {len(result.eigenvalues)} eigenvalues:")
    for i, lam in enumerate(result.eigenvalues):
        print(f"  \lambda[{10+i}] = {lam:.8f}")
