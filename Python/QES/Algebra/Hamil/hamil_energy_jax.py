...
def local_energy_jax_wrap(
    ...
    operator_terms_list
        List of modifying operator terms with site indices.
    operator_terms_list_ns
        List of modifying operator terms without site indices.
    operator_terms_list_nmod
        List of non-modifying operator terms with site indices.
    operator_terms_list_nmod_ns
        List of non-modifying operator terms without site indices.
    n_max
    ...
    def init_wrapper():
        ...
        fallback_nmod_sites_mults_arr   = (
                                            ...
                                            else jnp.array([], dtype=dtype) # Empty array
                                        )
        ...
        def wrapper(state: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
            ...
            diag    = jnp.zeros((1,), dtype=dtype) # sum over contributions
            ...
            for f, indices, mults in nmod_sites_data:
                ...
                def apply_op_nmod(s, idxs, op_func=f):
                    if idxs.ndim == 0:
                        return op_func(s, idxs)
                    return op_func(s, *idxs)
            ...
            for f, indices, mults in mod_sites_data:
                ...
                def apply_op_mod(s, idxs, op_func=f):
                    ...
                    if idxs.ndim == 0:
                        new_states, coeffs = op_func(s, idxs)
                    else:
                        new_states, coeffs = op_func(s, *idxs)
