...
class DiagonalizationEngine:
    ...
    def diagonalize(
        ...
        **kwargs,
        ...
        self._k = (
            k if k is not None else (self._n if self.method == DiagonalizationMethods.EXACT else 6)
        )
        method = self.method
        if method == "auto":
    ...
    def _extract_krylov_basis(self, A: Optional[NDArray], matvec: Optional[Callable]):
        ...
        self._krylov_basis = (
            self._result.subspacevectors if hasattr(self._result, "subspacevectors") else None
        )
