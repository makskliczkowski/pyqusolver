...
def test_engine_basic():
    ...
    try:
        ...
        result  = choose_eigensolver('lanczos', A, k=k, which='smallest', 
                                   hermitian=True, use_scipy=True)
...
def demonstrate_usage():
    ...
    A       = sp.diags([np.arange(n), np.ones(n-1), np.ones(n-1)],
                       [0, 1, -1], format='csr')
